...
function sortItems(items: CollectionItemWithStatus[], sortBy: string): CollectionItemWithStatus[] {
  return [...items].sort((a, b) => {
    switch (sortBy) {
      ...
      case "rarity":         return (a.dropRarityNum ?? 1) - (b.dropRarityNum ?? 1);
...
export function CategoryDetailContent({ username, categorySlug }: Props) {
  ...
  const osrsCat = OSRS_CATEGORIES.find((c) => c.slug === categorySlug);
  ...
  // Fallback: RuneLite sprite of the category icon item
  const iconSpriteUrl = osrsCat?.iconItemId
    ? `https://static.runelite.net/cache/item/icon/${osrsCat.iconItemId}.png`
    : null;
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-5">
          ...
          <div className="flex flex-col sm:flex-row sm:items-start gap-4">
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-1">
                {/* If no wiki image, show RuneLite sprite next to title */}
                {(bossImgError || !bossImageUrl) && iconSpriteUrl && (
                  <Image
                    src={iconSpriteUrl}
                    alt={cat.name}
                    width={32}
      ...
      {items.length > 0 ? (
        ...
        <EmptyState
          ...
          action={{
            label: "Clear filters",
            onClick: () => { setSearch(""); setShowMissing(false); setShowObtained(false); },
          }}
        />
      )}
